...
assert.match(reviewFlow, /window\.setTimeout\([\s\S]*1000/);
assert.match(reviewFlow, /clearRatingTransition/);
assert.match(reviewFlow, /const goBack[\s\S]*clearRatingTransition\(\);[\s\S]*setStep\(previousStep\)/);
assert.match(reviewFlow, /const commentTextareaRef = useRef<HTMLTextAreaElement>\(null\)/);
assert.match(reviewFlow, /const shouldFocusCommentRef = useRef\(false\)/);
assert.match(reviewFlow, /shouldFocusCommentRef\.current = true;[\s\S]*setStep\("comment"\)/);
assert.match(reviewFlow, /const stepHeadingRef = useRef<HTMLHeadingElement>\(null\)/);
assert.match(reviewFlow, /if \(isOpen\) return;[\s\S]*clearRatingTransition\(\);[\s\S]*shouldFocusCommentRef\.current = false;[\s\S]*\}, \[isOpen\]\)/);
assert.match(reviewFlow, /if \(!isOpen\) return;[\s\S]*step === "comment" && shouldFocusCommentRef\.current[\s\S]*commentTextareaRef\.current\?\.focus\(\);[\s\S]*stepHeadingRef\.current\?\.focus\(\)/);
assert.match(reviewFlow, /ref=\{commentTextareaRef\}/);
